...
class StartupChecker:
    ...
    def check_python_version(self) -> bool:
        ...
        if version.major >= required_major and version.minor >= required_minor:
            ...
            self.print_status(f"Python {version.major}.{version.minor}.{version.micro} is too old (need >= {required_major}.{required_minor})", "FAIL")
            self.errors.append(f"Upgrade Python to {required_major}.{required_minor} or higher")
            return False
    ...
    def check_required_packages(self) -> bool:
        ...
        if missing_packages:
            self.print_status(f"Missing {len(missing_packages)} required packages:", "FAIL")
            for package, description in missing_packages:
                self.print_status(f"  Missing: {package} - {description}", "FAIL")
            self.errors.append("Install missing packages with: pip install -r simple_requirements.txt")
            return False
    ...
    def check_ollama_service(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.print_status(f"Ollama service returned status {response.status_code}", "FAIL")
    ...
    def check_ollama_models(self) -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                self.print_status("Cannot check models - Ollama service not available", "FAIL")
                return False
            ...
            if not available_models:
                ...
                self.warnings += 1
                self.errors.append("Download a model: ollama pull llama2 (or your preferred model)")
                return False
    ...
    def check_storage_directories(self) -> bool:
        ...
        base_dir = Path("C:/Users/THE/open-source-rag-system")
        required_dirs = [
            base_dir / "storage",
                ...
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    if result == 0:
                        if port == 11434:  # Ollama should be running
                            self.print_status(f"  Port {port} ({service}) - in use (expected)", "PASS")
                        else:  # API port should be free
                            self.print_status(f"  Port {port} ({service}) - in use (may conflict)", "WARN")
                            self.warnings += 1
                    else:
                        if port == 8001:  # API port should be free
                            self.print_status(f"  Port {port} ({service}) - available", "PASS")
                        else:  # Ollama should be running
                            self.print_status(f"  Port {port} ({service}) - not available", "FAIL")
                            all_available = False
    ...
    def run_all_checks(self) -> Dict[str, bool]:
        ...
        if failed == 0:
            print(f"{Colors.GREEN}[OK] All checks passed! System ready to start.{Colors.END}")
            if self.warnings > 0:
                print(f"{Colors.YELLOW}[WARN] {self.warnings} warnings found{Colors.END}")
        else:
            print(f"{Colors.RED}[FAIL] {failed} checks failed. System cannot start.{Colors.END}")
            if self.warnings > 0:
                print(f"{Colors.YELLOW}[WARN] {self.warnings} warnings found{Colors.END}")
...
def main():
    ...
    all_critical_passed = all(results[key] for key in ['python_version', 'required_packages', 'storage_directories'])
