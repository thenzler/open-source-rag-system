        ...
        with self._lock:
            ...
            if inspect.isclass(implementation) and hasattr(implementation, '__init__'):
                try:
                    ...
                    # Skip 'self' parameter
                    dependencies = {name: hint for name, hint in type_hints.items() if name != 'return'}
                except Exception as e:
                    logger.debug(f"Could not extract type hints for {implementation.__name__}: {e}")
            
            descriptor = ServiceDescriptor(service_type, implementation, lifetime, dependencies)
    ...
    def _get_scoped(self, descriptor: ServiceDescriptor, scope_id: str) -> Any:
        ...
        with self._lock:
            ...
            if descriptor.service_type not in scope_instances:
                ...
                logger.debug(f"Created scoped instance of {descriptor.service_type.__name__} for scope {scope_id}")
    ...
    def _create_instance(self, descriptor: ServiceDescriptor) -> Any:
        ...
        if callable(implementation) and not inspect.isclass(implementation):
            try:
                ...
                logger.error(f"Error creating instance via factory for {descriptor.service_type.__name__}: {e}")
                raise
                    ...
                    if param.annotation != inspect.Parameter.empty:
                        try:
                            ...
                            if param.default != inspect.Parameter.empty:
                                ...
                                logger.warning(f"Could not resolve dependency {param.annotation.__name__} for {implementation.__name__}")
                
                return implementation(**kwargs)
                
            except Exception as e:
                logger.error(f"Error creating instance of {implementation.__name__}: {e}")
                raise
    ...
    async def initialize_all(self) -> bool:
        ...
        try:
            with self._lock:
                ...
                logger.info(f"Successfully initialized {len(self._initialization_order)} services")
                return True
    ...
    async def shutdown_all(self):
        ...
        try:
            ...
            for service_type in reversed(self._initialization_order):
                ...
                if instance and hasattr(instance, 'shutdown') and callable(getattr(instance, 'shutdown')):
                    try:
        ...
        for param_name, param in sig.parameters.items():
            if param_name not in kwargs and param.annotation != inspect.Parameter.empty:
                try:
                    ...
                    logger.debug(f"Could not inject {param.annotation.__name__} into {func.__name__}: {e}")
