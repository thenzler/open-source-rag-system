...
from .container import DIContainer, get_container, Lifetime
...
try:
    ...
    config = None
from ..services import DocumentProcessingService, QueryProcessingService, ValidationService
...
class ServiceConfiguration:
    ...
    def configure_repositories(container: DIContainer) -> DIContainer:
        ...
        container.register_singleton(
            IDocumentRepository,
            lambda: get_rag_repository().documents
        )
        
        container.register_singleton(
            IVectorSearchRepository,
            lambda: get_rag_repository().vector_search
        )
        
        container.register_singleton(
            SwissAuditRepository,
            lambda: get_rag_repository().audit
        )
