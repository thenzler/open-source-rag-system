...
class MetricsMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, collect_detailed_metrics: bool = True):
        ...
        logger.info(
            f"Metrics middleware initialized (detailed: {collect_detailed_metrics})"
        )
...
class QueryMetricsCollector:
    ...
    def record_query_start(self, tenant: str = "default"):
        """Record query start"""
        return {"start_time": time.time(), "tenant": tenant, "components": {}}
    ...
    def record_query_end(
        self, context: dict, status: str = "success", relevance_score: float = None
    ):
