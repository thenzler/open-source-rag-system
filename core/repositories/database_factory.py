...
class DatabaseFactory:
    ...
    def create_document_repository(
        db_type: str = "sqlite", connection_string: Optional[str] = None, **kwargs
    ) -> IDocumentRepository:
        ...
        if db_type.lower() == "postgresql":
            if not POSTGRESQL_AVAILABLE:
                logger.warning(
                    "PostgreSQL requested but dependencies not available, falling back to SQLite"
                )
                return DatabaseFactory._create_sqlite_repository(
            ...
            return DatabaseFactory._create_postgresql_repository(
                connection_string, **kwargs
            )
    ...
    def from_config(config_obj) -> IDocumentRepository:
        ...
        try:
            ...
            return DatabaseFactory.create_document_repository(
                db_type="sqlite", connection_string=sqlite_path
            )
...
class DatabaseMigrator:
    ...
    def __init__(
        self, source_repo: IDocumentRepository, target_repo: IDocumentRepository
    ):
    ...
    async def migrate_documents(self, batch_size: int = 100) -> bool:
        ...
        try:
            ...
            while True:
                result = await self.source_repo.list_all(
            ...
            logger.info(
                f"Migration completed. Total documents migrated: {total_migrated}"
            )
            return True
...
def recommend_database_type(expected_users: int, expected_documents: int) -> str:
    ...
    if expected_users > 10 or expected_documents > 10000:
        if POSTGRESQL_AVAILABLE:
            ...
            logger.warning(
                "PostgreSQL recommended but not available, suggesting SQLite with warning"
            )
            return "sqlite"
    else:
        return "sqlite"
