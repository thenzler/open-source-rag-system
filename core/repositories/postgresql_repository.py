...
from .models import Document, DocumentChunk, Embedding, SearchResult, User, Tenant, QueryLog
...
class PostgreSQLRepository(IDocumentRepository):
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(f"PostgreSQL repository initialized with pool size {self.pool_size}")
    ...
    async def _create_tables(self):
        ...
        async with self.pool.acquire() as conn:
            ...
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_documents_upload_timestamp ON documents(upload_timestamp)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_chunks_document_id ON chunks(document_id)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_chunks_tenant_id ON chunks(tenant_id)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_embeddings_document_id ON embeddings(document_id)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_embeddings_tenant_id ON embeddings(tenant_id)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_users_tenant_id ON users(tenant_id)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_query_logs_tenant_id ON query_logs(tenant_id)")
    ...
    async def list_all(self, tenant_id: Optional[int] = None, limit: int = 100, offset: int = 0) -> RepositoryResult:
    ...
    async def update(self, document_id: int, updates: Dict[str, Any]) -> Optional[Document]:
        ...
        for key, value in updates.items():
            if key in ['filename', 'description', 'status', 'text_content', 'chunk_count', 'embedding_count']:
...
class PostgreSQLUserRepository:
    ...
    async def list_users(self, tenant_id: Optional[int] = None, limit: int = 100, offset: int = 0) -> List[User]:
        """List users"""
        async with self.pool.acquire() as conn:
            if tenant_id:
                rows = await conn.fetch("""
