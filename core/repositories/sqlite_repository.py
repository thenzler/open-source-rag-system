...
from .interfaces import IDocumentRepository, IChunkRepository, IEmbeddingRepository
from .models import Document, DocumentChunk, Embedding, DocumentStatus
    ...
    def _run_migrations(self, conn):
        ...
        try:
            ...
            if migrations_needed:
                conn.commit()
                logger.info(f"Database migrations completed: {len(migrations_needed)} columns added")
            else:
    ...
    def _init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            try:
                ...
                conn.execute("CREATE INDEX IF NOT EXISTS idx_documents_uploader ON documents (uploader)")
...
class SQLiteDocumentRepository(SQLiteRepository, IDocumentRepository):
    ...
    async def create(self, document: Document) -> Document:
        ...
        with self.get_connection() as conn:
            ...
            """, (
                ...
                document.file_size,
                document.status.value if document.status else DocumentStatus.UPLOADING.value,
                document.uploader,
    ...
    async def update(self, document_id: int, updates: Dict[str, Any]) -> Optional[Document]:
        """Update document"""
        # Build dynamic update query
        allowed_fields = {
            'status', 'processing_timestamp', 'completion_timestamp',
            'description', 'tags', 'metadata', 'text_content',
    ...
    async def list_all(self, options: Optional[SearchOptions] = None) -> QueryResult[Document]:
    ...
    def _safe_status(self, status_value):
        ...
        try:
            ...
            logger.warning(f"Unknown document status '{status_value}', defaulting to UPLOADING")
            return DocumentStatus.UPLOADING
    ...
    def _row_to_document(self, row) -> Document:
        ...
        return Document(
            ...
            upload_timestamp=safe_datetime(safe_get('upload_timestamp') or safe_get('created_at')),
    ...
    async def exists(self, document_id: int) -> bool:
        with self.get_connection() as conn:
            cursor = conn.execute("SELECT 1 FROM documents WHERE id = ?", (document_id,))
    ...
    async def get_by_uploader(self, uploader: str, options: Optional[SearchOptions] = None) -> QueryResult[Document]:
        ...
        with self.get_connection() as conn:
            # Count query
            count_cursor = conn.execute("SELECT COUNT(*) FROM documents WHERE uploader = ?", (uploader,))
            total_count = count_cursor.fetchone()[0]
