...
from .models import Document, DocumentChunk, Embedding, User, QueryLog
...
class IChunkRepository(BaseRepository[DocumentChunk, int]):
    ...
    @abstractmethod
    async def get_by_document_id(self, document_id: int, options: Optional[SearchOptions] = None) -> QueryResult[DocumentChunk]:
    ...
    @abstractmethod
    async def search_text(self, query: str, options: Optional[SearchOptions] = None) -> QueryResult[DocumentChunk]:
        """Full-text search in chunks"""
        pass
    
    @abstractmethod
    async def get_chunk_statistics(self, document_id: Optional[int] = None) -> Dict[str, Any]:
...
class IUserRepository(BaseRepository[User, int]):
    ...
    @abstractmethod
    async def get_active_users(self, options: Optional[SearchOptions] = None) -> QueryResult[User]:
...
class IQueryHistoryRepository(BaseRepository[QueryLog, int]):
    ...
    @abstractmethod
    async def get_by_user_id(self, user_id: int, options: Optional[SearchOptions] = None) -> QueryResult[QueryLog]:
