...
class FAISSIndex(VectorIndex):
    ...
    def add_vectors(self, vectors: np.ndarray, ids: List[int]):
        ...
        if hasattr(self.index, 'is_trained') and not self.index.is_trained:
            ...
            if normalized_vectors.shape[0] < 100:
                ...
                if self._next_position > 0:
                    logger.warning("Switching to flat index, existing vectors will be lost")
                    self._next_position = 0
                ...
                logger.warning(f"Not enough vectors for training IVF index ({normalized_vectors.shape[0]} < {self.index.nlist * 10}), switching to flat index")
                self.index = faiss.IndexFlatIP(self.dimension)
                # Re-add any existing vectors
                if self._next_position > 0:
                    logger.warning("Switching to flat index, existing vectors will be lost")
        ...
        logger.info(f"Added {len(ids)} vectors to FAISS index (total: {self._next_position})")
...
class NumpyIndex(VectorIndex):
    ...
    def add_vectors(self, vectors: np.ndarray, ids: List[int]):
        ...
        logger.info(f"Added {len(ids)} vectors to Numpy index (total: {len(self.vectors)})")
...
class ProductionVectorRepository(IVectorSearchRepository):
    ...
    async def search_similar(
        ...
        try:
            ...
            results = [(int(id_), float(sim)) for sim, id_ in zip(similarities, embedding_ids)]
    ...
    def _cache_result(self, cache_key: str, result: List[Tuple[int, float]], access_time: float):
        ...
        try:
            ...
            if vector_results:
                ...
                for embedding_id, similarity_score in vector_results:
                    logger.debug(f"Processing embedding {embedding_id}, similarity {similarity_score:.6f}, threshold {threshold}")
                    ...
                    if embedding_id in chunk_data:
                        if similarity_score >= threshold:
                            ...
                            chunks.append(chunk)
                            logger.debug(f"Added chunk {chunk_info['chunk_id']} from doc {chunk_info['document_id']}")
                        else:
                            logger.debug(f"Embedding {embedding_id} filtered out: similarity {similarity_score:.6f} < threshold {threshold}")
                    else:
                        logger.warning(f"No chunk data found for embedding {embedding_id}")
            
            logger.info(f"Text search returning {len(chunks)} chunks (threshold: {threshold})")
    ...
    async def _load_chunks_by_embedding_ids(self, embedding_ids: List[int]) -> Dict[int, Dict[str, Any]]:
    ...
    async def _load_existing_embeddings(self):
        ...
        try:
            ...
            if embeddings:
                ...
                if success:
                    logger.info(f"✅ Loaded {len(embeddings)} embeddings into vector index")
                else:
                    logger.error("❌ Failed to build vector index from existing embeddings")
