...
class BaseRepository(ABC, Generic[Entity, EntityId]):
    ...
    @abstractmethod
    async def update(self, entity_id: EntityId, updates: Dict[str, Any]) -> Optional[Entity]:
        """Update entity by ID"""
        pass
    ...
    @abstractmethod
    async def list_all(self, options: Optional[SearchOptions] = None) -> QueryResult[Entity]:
