...
class SwissAuditRepository:
    ...
    def _init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_audit_data_subject ON audit_log (data_subject_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_compliance_subject ON compliance_events (data_subject_id)")
    ...
    def _insert_default_retention_policies(self, conn):
        ...
        default_policies = [
            ...
            ('confidential', 'document_access', 2555, 730),  # 7 years for business documents
        ...
        try:
            ...
            with self.get_connection() as conn:
                ...
                """, (
                    ...
                    entry.session_id,
                    entry.document_id,
                    entry.data_classification.value if entry.data_classification else 'internal',
                    entry.legal_basis.value if entry.legal_basis else 'legitimate_interests',
                ...
                logger.debug(f"Logged audit event {entry.event_type.value if entry.event_type else 'unknown'} with ID {audit_id}")
                return audit_id
    ...
    async def get_document_access_history(self, document_id: int) -> List[Dict[str, Any]]:
