...
class OllamaClient:
    ...
    def __init__(self, 
                 base_url: str = None,
                 model: str = None,
                 timeout: int = 180):  # 3 minutes - buffer for slower machines
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = self.client.generate(
                    ...
                    prompt=prompt,
                    options={
                        'num_predict': min(max_tokens, 256),  # Ultra-aggressive token limit for speed
                        'temperature': min(temperature, 0.1),  # Ultra-low temp for fastest responses
                        'top_p': 0.9,
