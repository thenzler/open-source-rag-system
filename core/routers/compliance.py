...
from ..services.compliance_service import (
    get_compliance_service, DataCategory, ProcessingPurpose, DataSubjectRight
)
...
class DataProcessingRequest(BaseModel):
    ...
    data_subject_identifier: Optional[str] = Field(None, description="Identifier for the data subject")
    data_source: str = Field("api", description="Source of the data")
    legal_basis: str = Field("legitimate_interest", description="Legal basis for processing")
...
async def record_data_processing(request: DataProcessingRequest) -> ComplianceResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to record data processing: {str(e)}"
        )
...
async def record_consent(request: ConsentRequest) -> ComplianceResponse:
    ...
    try:
        ...
        # Validate inputs
        processing_purpose = validate_processing_purpose(request.processing_purpose)
        data_categories = [validate_data_category(cat) for cat in request.data_categories]
...
async def submit_data_subject_request(request: DataSubjectRequestModel) -> ComplianceResponse:
    """
