...
from ..services.async_processing_service import (
    get_async_processor, TaskStatus, TaskPriority, ProcessingTask
)
...
async def get_tenant_tasks(tenant_id: str, status: Optional[str] = None):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get tenant tasks: {str(e)}")
...
async def processing_health_check():
    ...
    try:
        ...
        if not processor.is_running:
            raise HTTPException(status_code=503, detail="Processing service is not running")
