    ...
    try:
        ...
        if metrics_service.enabled:
            config["registry_info"] = {
                ...
                "metric_families": list(metrics_service.registry._collector_to_names.values())
            }
...
async def get_sample_metrics():
    ...
    try:
        ...
        sample = {
            ...
            "uptime_seconds": metrics_service.get_stats_summary().get("uptime_seconds", 0),
            "metrics_preview": []
        }
        ...
        sample["total_metrics_lines"] = len([l for l in lines if l.strip() and not l.startswith('#')])
