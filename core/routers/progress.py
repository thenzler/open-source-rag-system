...
async def websocket_progress_updates(
    ...
    try:
        ...
        if operation:
            ...
            await websocket.send_text(
                json.dumps(
                    {"type": "error", "message": f"Operation {operation_id} not found"}
                )
            )
        ...
        while True:
            try:
                ...
                if data.get("type") == "ping":
                    await websocket.send_text(
                        json.dumps({"type": "pong", "timestamp": operation.started_at})
                    )
...
async def get_operation(
    operation_id: str, tracker: ProgressTracker = Depends(get_tracker)
):
...
async def list_operations(
    ...
    try:
        ...
        operations = tracker.list_operations(
            tenant_id=tenant_id, user_id=user_id, status=status_filter
        )
...
@router.post(
    "/operations/{operation_id}/steps/{step_index}/complete", response_model=dict
)
...
async def cleanup_old_operations(
    max_age_hours: int = Query(
        168, description="Maximum age in hours (default: 7 days)"
    ),
    tracker: ProgressTracker = Depends(get_tracker),
):
...
async def progress_health_check(tracker: ProgressTracker = Depends(get_tracker)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail="Progress tracking service unavailable"
        )


@router.get("/stats", response_model=dict)
async def progress_statistics(tracker: ProgressTracker = Depends(get_tracker)):
    ...
    try:
        ...
        stats = {
            ...
            "active_websockets": sum(
                len(conns) for conns in tracker.websocket_connections.values()
            ),
            "operations_with_listeners": len(
                [
                    op_id
                    for op_id, conns in tracker.websocket_connections.items()
                    if len(conns) > 0
                ]
            ),
        }
