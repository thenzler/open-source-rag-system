...
def get_repositories():
    ...
    try:
        from ..repositories.factory import get_rag_repository, get_document_repository, get_vector_search_repository
        return {
    ...
    try:
        ...
        if not dry_run:
            ...
            for analysis in problematic_docs:
                ...
                if remove_training_docs and "training_instructions" in analysis.problematic_reasons:
                    should_remove = True
                if remove_computer_science and "computer_science" in analysis.problematic_reasons:
                    should_remove = True
                if remove_corrupted and "corrupted_encoding" in analysis.problematic_reasons:
                    should_remove = True
                ...
                if should_remove:
                    try:
                        ...
                        if success:
                            ...
                            logger.info(f"Removed problematic document {analysis.document_id}: {analysis.filename}")
                        else:
                            logger.error(f"Failed to remove document {analysis.document_id}: delete returned False")
                    except Exception as e:
                        logger.error(f"Failed to remove document {analysis.document_id}: {e}")
...
async def rebuild_vector_index():
    ...
    try:
        ...
        analyses = await analyze_all_documents()
        clean_docs = [a for a in analyses if not a.is_problematic and a.content_type == "bio_waste"]
...
async def get_bio_waste_documents():
    ...
    try:
        analyses = await analyze_all_documents()
        bio_waste_docs = [a for a in analyses if a.content_type == "bio_waste" and not a.is_problematic]
        
        return {
            "count": len(bio_waste_docs),
            "documents": bio_waste_docs
        }
...
def _analyze_content_quality(content: str) -> tuple[bool, List[str]]:
    ...
    if any(keyword in content_lower for keyword in config.get('problematic_keywords', [])):
...
def _classify_content_type(content: str) -> str:
    ...
    cs_indicators = ['javascript', 'programming', 'software', 'algorithm', 'cloud computing']
...
def _calculate_confidence(content: str) -> float:
    ...
    bio_score = 0
    bio_keywords = ['bioabfall', 'organic', 'compost', 'obst', 'gemüse', 'küchenabfälle']
...
async def _rebuild_clean_vector_index(repos: Dict, clean_documents: List[DocumentAnalysis]):
    """Rebuild vector index with only clean documents"""
    try:
        vector_repo = repos['vector_repo']
        
        logger.info(f"Rebuilding vector index with {len(clean_documents)} clean documents")
        ...
        for doc_analysis in clean_documents:
            logger.info(f"Re-indexing document {doc_analysis.document_id}: {doc_analysis.filename}")
