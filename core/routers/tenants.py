...
async def create_tenant(
    ...
    try:
        ...
        tenant = Tenant(
            ...
            or {"max_documents": 1000, "max_storage_mb": 1024, "max_users": 10},
        )
...
async def list_tenants(tenant_repo: TenantRepository = Depends(get_tenant_repository)):
...
async def get_tenant(
    tenant_id: int, tenant_repo: TenantRepository = Depends(get_tenant_repository)
):
...
async def delete_tenant(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail="Tenant not found or cannot be deleted"
            )
