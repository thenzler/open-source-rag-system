...
async def flush_cache(
    tenant_id: Optional[str] = Query(None, description="Flush cache for specific tenant only"),
    ...
    try:
        ...
        return {
            'message': f'Cache flushed successfully for tenant: {target_tenant}' if target_tenant else 'Entire cache flushed successfully',
...
async def invalidate_pattern(
    ...
    tenant_id: Optional[str] = Query(None, description="Tenant ID for scoped invalidation"),
    current_tenant: Optional[str] = Depends(get_current_tenant),
    cache: Optional[RedisCacheService] = Depends(get_cache)
):
...
async def invalidate_tenant_cache(
    ...
    if current_tenant and tenant_id != current_tenant:
        raise HTTPException(status_code=403, detail="Can only invalidate current tenant's cache")
...
async def get_cached_query(
    ...
    try:
        cached_result = await cache.get(CacheKeyType.QUERY_RESULT, query_hash, tenant_id)
...
async def test_cache_operations(
    ...
    try:
        ...
        # Test set
        set_success = await cache.set(CacheKeyType.SYSTEM_CONFIG, test_key, test_value, 60)
        if not set_success:
...
async def get_cache_configuration(
    ...
    return {
        'available': cache.is_available,
        'config': {
            'redis_url': cache.redis_url.replace(cache.redis_url.split('@')[-1].split('/')[0], '***') if '@' in cache.redis_url else cache.redis_url,
