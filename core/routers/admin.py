...
async def admin_dashboard(request: Request):
    ...
    try:
        ...
        return templates.TemplateResponse("admin_error.html", {
            "request": request,
            "error": str(e)
        })
    ...
    try:
        ...
        for model_key, model_config in models.items():
            try:
                ...
                models[model_key]['status'] = 'available' if temp_client.is_available() else 'not_downloaded'
...
async def switch_model(selection: ModelSelection):
    ...
    try:
        ...
        if selection.model_key not in config.get('models', {}):
            raise HTTPException(status_code=400, detail=f"Model '{selection.model_key}' not found")
        ...
        # Log the change
        logger.info(f"Model switched from {old_model} to {selection.model_key} (reason: {selection.reason})")
...
async def install_model(model_key: str):
    ...
    try:
        ...
        if model_key not in config.get('models', {}):
            raise HTTPException(status_code=400, detail=f"Model '{model_key}' not found in config")
...
async def download_config():
    ...
    try:
        ...
        return Response(
            ...
            headers={"Content-Disposition": "attachment; filename=llm_config_backup.yaml"}
        )
...
async def get_recent_logs():
    ...
    try:
        ...
        return {
            "logs": [
                {"timestamp": "2024-01-28 10:30:00", "level": "INFO", "message": "System started"},
                {"timestamp": "2024-01-28 10:31:00", "level": "INFO", "message": "Model loaded: command-r7b"},
                {"timestamp": "2024-01-28 10:32:00", "level": "INFO", "message": "Admin interface accessed"}
            ],
            "total_count": 3
        }
    ...
    try:
        ...
        analyses_dict = {
            ...
            "documents": {
                ...
                "problematic": [doc.dict() for doc in analyses["documents"]["problematic"]],
...
async def test_database_connection(config: DatabaseConfig):
    ...
    try:
        if config.db_type == "sqlite":
            ...
            return {"success": False, "message": f"Unsupported database type: {config.db_type}"}
