...
class DocumentProcessors:
    ...
    async def process_upload(
        self, task: ProcessingTask, update_progress: Callable
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self.doc_metrics.record_processing(
                file_path.suffix.lower().replace(".", ""), "success", processing_time
            )
            ...
            self.doc_metrics.record_processing(
                Path(task.file_path).suffix.lower().replace(".", ""), "failed", 0
            )
    ...
    async def _store_document(
        self, task: ProcessingTask, content: str, embeddings: list
    ) -> Dict[str, Any]:
    ...
    async def _update_document_index(
        self, document_id: int, embeddings: list
    ) -> Dict[str, Any]:
    ...
    async def _process_single_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        return {"content_length": len(content), "embeddings_count": len(embeddings)}
...
async def register_document_processors():
    ...
    processor_service.register_processor(
        "batch_import", processors.process_batch_import
    )
