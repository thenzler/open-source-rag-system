...
class BackupService:
    ...
    def __init__(
        ...
        logger.info(f"Backup service initialized: {self.backup_dir} (retention: {retention_days} days)")
    ...
    async def _create_backup_manifest(self, backup_path: Path, backup_info: Dict[str, Any]):
                ...
                if item.is_dir() and item.name.startswith('rag_backup_'):
                    ...
                    if manifest_file.exists():
                        ...
                        backups.append({
                            "name": item.name,
                            "path": str(item),
                            "timestamp": manifest.get("backup_info", {}).get("timestamp"),
                            "compressed": False,
                            "size_bytes": self._get_directory_size(item),
                            "manifest": manifest
                        })
                
                elif item.is_file() and item.name.startswith('rag_backup_') and item.suffix == '.gz':
                    # Compressed backup
                    backups.append({
                        "name": item.stem,
                        "path": str(item),
                        "timestamp": item.stem.split('_')[-1] if '_' in item.stem else None,
                        "compressed": True,
    ...
    async def restore_backup(self, backup_name: str, components: List[str] = None) -> Dict[str, Any]:
        """Restore from backup"""
        ...
        try:
            ...
            if manifest_file.exists():
                ...
                logger.warning("No backup manifest found, proceeding with best effort restore")
                manifest = {}
            ...
            for component in components:
                try:
                    ...
                    restore_info["errors"].append({"component": component, "error": str(e)})
            ...
            logger.info(f"Restore completed: {backup_name} (success: {restore_info['success']})")
    ...
    async def cleanup_old_backups(self) -> Dict[str, Any]:
            ...
            for item in self.backup_dir.iterdir():
                if item.name.startswith('rag_backup_'):
                    ...
                    try:
                        ...
                        logger.warning(f"Could not parse backup date from {item.name}: {e}")
        ...
        logger.info(f"Backup cleanup completed: {len(deleted)} deleted, {len(errors)} errors")
