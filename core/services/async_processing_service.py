...
class AsyncDocumentProcessor:
    ...
    async def _process_task(self, worker_name: str, task: ProcessingTask):
        ...
        try:
            ...
            logger.info(
                f"Worker {worker_name} processing task {task_id} ({task.task_type})"
            )
            ...
            processing_task = asyncio.create_task(self._run_processor(processor, task))
...
async def initialize_async_processor(
    max_workers: int = 4, enable_persistence: bool = True
) -> AsyncDocumentProcessor:
    """Initialize and start the global async processor"""
    global _processor
    _processor = AsyncDocumentProcessor(
        max_workers=max_workers, enable_persistence=enable_persistence
    )
