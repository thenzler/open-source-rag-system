...
class SwissDataProtectionService:
    ...
    def __init__(
        ...
        logger.info(
            f"Swiss Data Protection Service initialized (region: {data_residency_region})"
        )
    ...
    async def record_data_processing(
        ...
        if custom_retention_days:
            ...
            default_retention = self.swiss_retention_defaults.get(
                data_category, timedelta(days=365)
            )
    ...
    async def process_right_to_access(self, request_id: str) -> Dict[str, Any]:
        ...
        for record in self.processing_records.values():
            if (
                ...
                and record.data_subject_id == request.data_subject_id
            ):
        ...
        for consent in self.consent_records.values():
            if (
                consent.tenant_id == request.tenant_id
                and consent.data_subject_id == request.data_subject_id
            ):
                data_subject_data["consents"].append(
                    {
                        ...
                        "data_categories": [
                            cat.value for cat in consent.data_categories
                        ],
                    }
                )
    ...
    async def cleanup_expired_data(self) -> Dict[str, int]:
        ...
        for request_id, request in self.data_subject_requests.items():
            if request.completed_at and request.completed_at < archive_cutoff:
    ...
    async def get_compliance_report(self, tenant_id: str) -> Dict[str, Any]:
        ...
        report = {
            ...
            "retention_compliance": {"expiring_soon": [], "overdue": []},
        }
    ...
    def _load_compliance_data(self):
        ...
        try:
            ...
            for request_id, request_data in data.get(
                "data_subject_requests", {}
            ).items():
                self.data_subject_requests[request_id] = self._dict_to_request(
                    request_data
                )

            logger.info(
                f"Loaded compliance data: {len(self.processing_records)} records, "
