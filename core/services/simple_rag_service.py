...
class RAGConfig:
    ...
    def __init__(self):
        ...
        self.require_sources = bool(os.getenv('RAG_REQUIRE_SOURCES', 'true').lower() == 'true')
        self.max_query_length = int(os.getenv('RAG_MAX_QUERY_LENGTH', '500'))
        
        logger.info(f"RAG Config: threshold={self.similarity_threshold}, max_results={self.max_results}, require_sources={self.require_sources}")
    ...
    async def _search_documents(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for item in search_results.items:
                ...
                if ('only use information' in content_lower or 
                    'zero-hallucination' in content_lower or
                    'guidelines for following' in content_lower or
                    'zusätzliche richtlinien' in content_lower or
                    item.document_id == 60):
                    logger.info(f"Skipping document {item.document_id} (contains training instructions)")
                    continue
                
                # EXCLUDE computer science content that doesn't contain bio waste info
                if ('javascript' in content_lower or 
                    'console.log' in content_lower or
                    'cloud computing' in content_lower or
                    '52 stunden informatik' in content_lower):
                    logger.info(f"Skipping document {item.document_id} (computer science content)")
                    continue
            ...
            logger.info(f"Found {len(relevant_results)} relevant documents (threshold: {self.config.similarity_threshold})")
...
ANSWER:"""
            ...
            result = {
                ...
                "debug_context": context[:500] + "..." if len(context) > 500 else context,
    ...
    def _audit_successful_query(self, query: str, response: Dict[str, Any]):
        ...
        try:
            if self.audit_repo:
                ...
                logger.info(f"Query audit: query_length={len(query)}, sources={len(response.get('sources', []))}, confidence={response.get('confidence', 0.0)}")
        except Exception as e:
