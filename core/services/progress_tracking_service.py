...
class ProgressTracker:
    ...
    def __init__(self, persistence_file: str = "data/progress_operations.json"):
        ...
        self.websocket_connections: Dict[str, Set] = {}  # operation_id -> set of websockets
        self.operation_callbacks: Dict[str, List[Callable]] = {}  # operation_id -> callbacks
    ...
    async def update_step_progress(
        ...
        async with self._lock:
            ...
            operation.overall_progress = total_progress / operation.total_steps if operation.total_steps > 0 else 0.0
            ...
            if operation.status == ProgressStatus.RUNNING and operation.started_at:
                ...
                if operation.overall_progress > 0:
                    ...
                    operation.estimated_completion = operation.started_at + estimated_total
    ...
    async def complete_step(self, operation_id: str, step_index: int, metadata: Optional[Dict[str, Any]] = None) -> bool:
        """Mark a step as completed"""
        return await self.update_step_progress(
            operation_id, step_index, 1.0, ProgressStatus.COMPLETED, metadata=metadata
        )
    
    async def fail_step(self, operation_id: str, step_index: int, error: str, metadata: Optional[Dict[str, Any]] = None) -> bool:
        """Mark a step as failed"""
        return await self.update_step_progress(
            operation_id, step_index, None, ProgressStatus.FAILED, error=error, metadata=metadata
        )
    
    async def cancel_operation(self, operation_id: str, reason: str = "Cancelled by user") -> bool:
...
async def initialize_progress_tracker(persistence_file: str = "data/progress_operations.json") -> ProgressTracker:
...
class ProgressContext:
    ...
    async def update_progress(self, progress: float, metadata: Optional[Dict[str, Any]] = None):
