...
class MetricsService:
    ...
    def _init_rag_metrics(self):
        ...
        self.vector_index_size = Gauge(
            "vector_index_size", "Vector index size", registry=self.registry
        )
    ...
    def record_document_processing(self, doc_type: str, status: str, duration: float):
        ...
        with self._lock:
            self.documents_processed_total.labels(status=status, type=doc_type).inc()

            if duration > 0:
                self.document_processing_duration.labels(type=doc_type).observe(
                    duration
                )
    ...
    def record_query(
        self, tenant: str, status: str, component_durations: Dict[str, float]
    ):
        ...
        with self._lock:
            self.queries_total.labels(tenant=tenant, status=status).inc()
    ...
    def record_llm_request(
        ...
        with self._lock:
            ...
            if input_tokens > 0:
                self.llm_token_count.labels(model=model, type="input").inc(input_tokens)

            if output_tokens > 0:
                self.llm_token_count.labels(model=model, type="output").inc(
                    output_tokens
                )
