...
class QueryProcessingService:
    ...
    async def validate_query(self, query: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            external_knowledge_detected, external_reason = (
                self.confidence_manager.detect_external_knowledge(query)
            )
            if external_knowledge_detected:
                ...
                return (
                    False,
                    f"Diese Frage erfordert externes Wissen außerhalb der verfügbaren Dokumente: {external_reason}",
                )
    ...
    async def search_documents(
        ...
        try:
            ...
            confidence_tiers = self.confidence_manager.determine_confidence_tiers(
                query, use_llm
            )
            ...
            vector_search_threshold = (
                0.05  # Very low to capture all potentially relevant results
            )
            ...
            search_results = await self.vector_repo.search_similar_text(
                query=query, limit=limit, threshold=vector_search_threshold
            )
            ...
            if processed_results["should_refuse"]:
                ...
                await self._audit_low_confidence_refused(
                    query,
                    processed_results["max_similarity"],
                    confidence_tiers["refusal_threshold"],
                    len(search_results.items),
                )
            ...
            should_use_llm = (
                ...
                and processed_results["confidence_tier"] in ["high", "medium"]
            )

            if should_use_llm:
                try:
                    ai_response_data = await self._generate_ai_response(
                    ...
                    logger.warning(
                        f"LLM processing failed, falling back to vector search: {e}"
                    )

            # Log successful query
            logger.info(
                f"Query completed: {len(formatted_results)} results ({response_data['search_type']})"
            )
...
"""
            ...
            validation_result = self._validate_ai_response(
                complete_answer, source_references
            )

            # If validation fails, return refusal instead of potentially hallucinated content
            if not validation_result.get("is_valid", False):
                logger.warning(
                    f"AI response failed validation: {validation_result.get('reason', 'unknown')}"
                )
            ...
            return {
                ...
                "sources": source_references,
                "confidence": (
                    min([r["similarity"] for r in source_references])
        ...
        try:
            ...
            if query_strategy == "factual":
                ...
                results = await self.search_documents(
                    query=request.query, limit=request.limit, use_llm=True
                )
    ...
    async def _process_results_with_confidence_tiers(
        self, search_items: List[Any], confidence_tiers: Dict[str, float], query: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if max_similarity >= confidence_tiers["high_threshold"]:
                ...
                filtered_results = [
                    ...
                    if r["similarity"] >= confidence_tiers["medium_threshold"]
                ]
                ...
                filtered_results = [
                    r
                    for r in all_results
                    if r["similarity"] >= confidence_tiers["low_threshold"]
                ]
                ...
                filtered_results = [
                    r
                    for r in all_results
                    if r["similarity"] >= confidence_tiers["refusal_threshold"]
                ]
            ...
            logger.info(
                f"Confidence tier: {confidence_tier}, max similarity: {max_similarity:.3f}, returning {len(filtered_results)} results"
            )
    ...
    def _generate_refusal_message(
        self, query: str, max_similarity: float, total_results: int
    ) -> str:
    ...
    def _validate_ai_response(
        self, ai_response: str, source_chunks: List[Dict]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_result = validator.validate_response_content(
                llm_response=ai_response, source_chunks=source_chunks
            )
            ...
            overall_valid = validation_result.get(
                "is_valid", False
            ) and citation_validation.get("is_valid", False)
    ...
    async def _audit_low_confidence_refused(
        self, query: str, max_similarity: float, threshold: float, result_count: int
    ):
    ...
    async def _audit_response_validation_failed(
        self, query: str, validation_reason: str, confidence_score: float
    ):
    ...
    async def _audit_llm_response_generated(
        self,
        query: str,
        response: str,
        source_count: int,
