...
class ConfidenceManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        logger.info(
            f"Confidence Manager initialized (enabled: {self.config.get('enabled', True)})"
        )
    ...
    def determine_confidence_tiers(
        ...
        try:
            ...
            for category, settings in irrelevant_topics.items():
                ...
                if any(term in query_lower for term in terms):
                    ...
                    logger.debug(
                        f"Applied irrelevant topic adjustment: {category} (+{adjustment})"
                    )
            ...
            if not applied_categories:  # Only if no irrelevant topics found
                ...
                if best_match:
                    ...
                    logger.debug(
                        f"Applied domain adjustment: {best_match} ({adjustment})"
                    )
            ...
            if applied_categories and self.config.get("logging", {}).get(
                "log_threshold_adjustments", True
            ):
                logger.info(
                    f"Query analysis applied: {', '.join(applied_categories)} (total adjustment: {total_adjustment:+.3f})"
                )
            ...
            for category, settings in external_config.items():
                ...
                for pattern in patterns:
                    if re.search(pattern, query_lower):
                        logger.info(
                            f"External knowledge detected: {reason} in category {category_name}"
                        )
    ...
    def get_status_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "base_thresholds": self.config.get("base_thresholds", {}),
                "query_analysis_enabled": self.config.get("query_analysis", {}).get(
                    "enabled", True
                ),
                "enabled_domains": enabled_domains,
                "disabled_domains": disabled_domains,
                "external_knowledge_enabled": self.config.get(
                    "external_knowledge", {}
                ).get("enabled", True),
