...
class RedisCacheService:
    ...
    def _generate_key(
        self, key_type: CacheKeyType, identifier: str, tenant_id: Optional[str] = None
    ) -> str:
    ...
    async def set_query_cache(
        ...
        return await self.set(
            CacheKeyType.QUERY_RESULT, cache_key, cached_result, ttl, tenant_id
        )

    async def get_document_embedding(
        self, document_id: str, model_name: str, tenant_id: Optional[str] = None
    ) -> Optional[np.ndarray]:
        """Get cached document embedding"""
        cache_key = f"{document_id}:{model_name}"
        cached_data = await self.get(
            CacheKeyType.DOCUMENT_EMBEDDING, cache_key, tenant_id
        )
    ...
    async def set_document_embedding(
        ...
        return await self.set(
            CacheKeyType.DOCUMENT_EMBEDDING, cache_key, cached_data, None, tenant_id
        )
    ...
    async def set_search_results(
        ...
        return await self.set(
            CacheKeyType.SEARCH_RESULT, cache_key, cached_data, None, tenant_id
        )
    ...
    async def get_llm_response(
        ...
        cache_data = {"prompt": prompt, "model": model_name, "temperature": temperature}
        cache_key = self._hash_key(cache_data)
    ...
    async def set_llm_response(
        ...
        return await self.set(
            CacheKeyType.LLM_RESPONSE, cache_key, cached_response, None, tenant_id
        )
...
async def initialize_cache_service(
    redis_url: str = "redis://localhost:6379", redis_db: int = 0, **kwargs
) -> Optional[RedisCacheService]:
    """Initialize global cache service"""
    global _cache_service

    _cache_service = RedisCacheService(redis_url=redis_url, redis_db=redis_db, **kwargs)
