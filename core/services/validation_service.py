...
try:
    ...
    logging.warning(
        "scikit-learn not available, response validation will use basic methods"
    )
...
class ValidationService:
    ...
    def validate_filename(self, filename: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if len(filename) > self.max_filename_length:
                return (
                    False,
                    f"Filename too long. Maximum {self.max_filename_length} characters",
                )
    ...
    def sanitize_string(
        self, input_string: str, max_length: Optional[int] = None
    ) -> str:
            ...
            for bool_param in ["use_llm", "include_metadata"]:
                if bool_param in params:
                    if isinstance(params[bool_param], bool):
                        ...
                        validated_params[bool_param] = params[bool_param].lower() in [
                            "true",
                            "1",
                            "yes",
                        ]
    ...
    def check_rate_limit_compliance(
        self, operation: str, user_id: str = "anonymous"
    ) -> Tuple[bool, str]:
                    ...
                    for pattern in self.sql_injection_patterns:
                        if re.search(pattern, value, re.IGNORECASE):
                            report["warnings"].append(
                                f"SQL injection pattern detected in {key}"
                            )
    ...
    def validate_response_content(
        self, llm_response: str, source_chunks: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
    ...
    def _clean_response_for_validation(self, response: str) -> str:
        ...
        try:
            ...
            response = re.sub(
                r"Download:.*?(/api/v1/documents/\d+/download)", "", response
            )
