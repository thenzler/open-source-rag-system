...
async def test_basic_cache_operations(cache_service, mock_redis):
    ...
    with patch.object(cache_service, '_serialize_value', return_value=serialized_data), \
        ...
        # Test set
        success = await cache_service.set(CacheKeyType.QUERY_RESULT, "test_key", test_value)
        assert success is True
...
async def test_query_cache(cache_service, mock_redis):
    ...
    with patch.object(cache_service, '_serialize_value', return_value=serialized_data), \
         patch.object(cache_service, '_deserialize_value', return_value={'result': result, 'cached_at': 1234567890, 'query_hash': 'abc123'}):
        ...
        mock_redis.get.return_value = None
        cached_result = await cache_service.get_query_cache("different query", filters=filters)
...
async def test_document_embedding_cache(cache_service, mock_redis):
    ...
    with patch.object(cache_service, '_serialize_value', return_value=serialized_data), \
        ...
        success = await cache_service.set_document_embedding(document_id, model_name, embedding)
        assert success is True
        
        # Test get embedding cache (hit)
        mock_redis.get.return_value = serialized_data
        cached_embedding = await cache_service.get_document_embedding(document_id, model_name)
        ...
        mock_redis.get.return_value = None
        cached_embedding = await cache_service.get_document_embedding("different_doc", model_name)
...
async def test_llm_response_cache(cache_service, mock_redis):
    ...
    with patch.object(cache_service, '_serialize_value', return_value=serialized_data), \
        ...
        # Test set LLM cache
        success = await cache_service.set_llm_response(prompt, response, model_name, temperature)
        assert success is True
        
        # Test get LLM cache (hit)
        mock_redis.get.return_value = serialized_data
        cached_response = await cache_service.get_llm_response(prompt, model_name, temperature)
...
async def test_invalidation_patterns(cache_service, mock_redis):
    ...
    deleted_count = await cache_service.invalidate_pattern("test_pattern", "test_tenant")
    ...
    deleted_count = await cache_service.invalidate_document_cache("doc123", "test_tenant")
    assert deleted_count == 3
...
async def test_ttl_configuration(cache_service, mock_redis):
    ...
    with patch.object(cache_service, '_serialize_value', return_value=serialized_data):
        ...
        mock_redis.setex.assert_called_with("rag_cache:config:test", 600, serialized_data)
...
async def test_error_handling(cache_service, mock_redis):
    ...
    with patch.object(cache_service, '_serialize_value', side_effect=Exception("Serialization failed")):
