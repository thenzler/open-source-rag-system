...
async def test_list_operations(temp_tracker):
    ...
    op1 = await temp_tracker.create_operation(
        "Op 1", "Description 1", [{'name': 'Step', 'description': 'Step'}],
        tenant_id="tenant1", user_id="user1"
    )
    op2 = await temp_tracker.create_operation(
        "Op 2", "Description 2", [{'name': 'Step', 'description': 'Step'}],
        tenant_id="tenant2", user_id="user1"
    )
    op3 = await temp_tracker.create_operation(
        "Op 3", "Description 3", [{'name': 'Step', 'description': 'Step'}],
        tenant_id="tenant1", user_id="user2"
    )
...
async def test_estimated_completion():
    ...
    with tempfile.NamedTemporaryFile(suffix='.json', delete=False) as tmp:
        ...
        await tracker.update_step_progress(operation_id, 0, 1.0, ProgressStatus.COMPLETED)
...
async def test_invalid_operations(temp_tracker):
    ...
    success = await temp_tracker.update_step_progress(operation_id, 5, 0.5)  # Invalid index
    assert success is False
